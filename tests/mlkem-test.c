...
/* note: test-utils.h must be included after t-common.h */
#include "test-utils.h"

static void
test_hex_decoding (void)
{
  ...
  for (i = 0; i < 2; i++)
    {
      ...
      if (bin_len != sizeof (exp_result)
          || memcmp (exp_result, buffer, bin_len))
        {
...
static int
check_mlkem_gen_enc_dec (unsigned mlkem_bits, int do_print_secmem_peak_usages)
{
  ...
  if (!fname)
    {
      ...
      for (i = 0; i < sizeof (mlkem_kat_files) / sizeof (mlkem_kat_files[0]);
           i++)
        {
          printf ("starting KAT tests for ML-KEM with bit strength %u ",
                  mlkem_bits[i]);
