/* t-sphincsplus.c - Test the SPHINCS+ Signature algorithm
...
const char SPHINCSPLUS_MESSAGE_TMPL[] = "(data (flags eddsa) (value %b))";


#define GCRY_SPHINCSPLUS2_NBITS (1312 * 8) 10496
#define GCRY_SPHINCSPLUS3_NBITS (1952 * 8) 15616
#define GCRY_SPHINCSPLUS5_NBITS (2592 * 8) 20736
...
static int check_sphincsplus_roundtrip(size_t n_tests)
{
  ...
  for (int j = 0; j < sizeof(variants)/sizeof(variants[0]); j++)
  {
    ...
    if (rc)
      die ("error generating SPHINCS+ key: %s\n", gpg_strerror (rc));
    ...
    rc = gcry_sexp_build (&s_data,
          NULL,
          SPHINCSPLUS_MESSAGE_TMPL, msg_len, msg, NULL);
    if (rc)
    {
...
int check_test_vec_verify(unsigned char *pk, unsigned pk_len, unsigned char *m, unsigned m_len, unsigned char *sig, unsigned sig_len, const char* hashalg, const char* variant) {
  ...
  err = gcry_sexp_build (&data_sx,
        NULL,
        SPHINCSPLUS_MESSAGE_TMPL, m_len, m, NULL);
  ...
  err = gcry_sexp_build (&signature_sx,
      NULL,
      "(sig-val(sphincsplus(a %b)))", sig_len, sig, NULL);
