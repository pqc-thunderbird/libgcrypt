...
typedef struct
{
  ...
  unsigned int (*absorb) (KECCAK_STATE *hd,
                          ...
                          size_t nlanes,
                          int blocklanes);
  unsigned int (*extract) (KECCAK_STATE *hd,
                           unsigned int pos,
                           byte *outbuf,
                           unsigned int outlen);
} keccak_ops_t;
...
static unsigned int
keccak_extract64 (KECCAK_STATE *hd,
                  unsigned int pos,
                  byte *outbuf,
                  unsigned int outlen)
{
...
static const u32 round_consts_32bit[2 * 24]
    = {0x00000001UL, 0x00000000UL, 0x00000000UL, 0x00000089UL, 0x00000000UL,
       ...
       0x00008000UL, 0x00000000UL, 0x80008082UL};

static unsigned int
keccak_extract32bi (KECCAK_STATE *hd,
                    unsigned int pos,
                    byte *outbuf,
                    unsigned int outlen)
{
  ...
  for (i = pos; i < pos + outlen / 8 + !!(outlen % 8); i++)
    {
      ...
      x0 = t;
      t  = (x0 ^ (x0 >> 8)) & 0x0000FF00UL;
      x0 = x0 ^ t ^ (t << 8);
      t  = (x0 ^ (x0 >> 4)) & 0x00F000F0UL;
      x0 = x0 ^ t ^ (t << 4);
      t  = (x0 ^ (x0 >> 2)) & 0x0C0C0C0CUL;
      x0 = x0 ^ t ^ (t << 2);
      t  = (x0 ^ (x0 >> 1)) & 0x22222222UL;
      x0 = x0 ^ t ^ (t << 1);
      t  = (x1 ^ (x1 >> 8)) & 0x0000FF00UL;
      x1 = x1 ^ t ^ (t << 8);
      t  = (x1 ^ (x1 >> 4)) & 0x00F000F0UL;
      x1 = x1 ^ t ^ (t << 4);
      t  = (x1 ^ (x1 >> 2)) & 0x0C0C0C0CUL;
      x1 = x1 ^ t ^ (t << 2);
      t  = (x1 ^ (x1 >> 1)) & 0x22222222UL;
      x1 = x1 ^ t ^ (t << 1);
...
static inline void
absorb_lanes64_8 (u64 *dst, const byte *in)
{
...
static inline void
absorb_lanes64_4 (u64 *dst, const byte *in)
{
  asm ("movdqu 0*16(%[dst]), %%xmm0\n\t"
       "movdqu 0*16(%[in]), %%xmm4\n\t"
       "movdqu 1*16(%[dst]), %%xmm1\n\t"
...
static inline void
absorb_lanes64_2 (u64 *dst, const byte *in)
{
  asm ("movdqu 0*16(%[dst]), %%xmm0\n\t"
       "movdqu 0*16(%[in]), %%xmm4\n\t"
       "pxor %%xmm4, %%xmm0\n\t"
...
static inline void
absorb_lanes64_1 (u64 *dst, const byte *in)
{
  dst[0] ^= buf_get_le64 (in + 8 * 0);
}
...
#define ROL64(x, n)                                                           \
  (((x) << ((unsigned int)n & 63)) | ((x) >> ((64 - (unsigned int)(n)) & 63)))
...
#define ROL64(x, n)                                                           \
  ({                                                                          \
    u64 tmp = (x);                                                            \
    asm ("shldq %1, %0, %0" : "+r"(tmp) : "J"((n)&63) : "cc");                \
    tmp;                                                                      \
  })
...
#define ANDN64(x, y)                                                          \
  ({                                                                          \
    u64 tmp;                                                                  \
    asm ("andnq %2, %1, %0" : "=r"(tmp) : "r0"(x), "rm"(y));                  \
    tmp;                                                                      \
  })
...
#define ROL64(x, n)                                                           \
  ({                                                                          \
    u64 tmp;                                                                  \
    asm ("rorxq %2, %1, %0" : "=r"(tmp) : "rm0"(x), "J"(64 - ((n)&63)));      \
    tmp;                                                                      \
  })
...
extern ASM_FUNC_ABI unsigned int _gcry_keccak_f1600_state_permute64_avx512 (
    u64 *state, const u64 *rconst);

extern ASM_FUNC_ABI unsigned int _gcry_keccak_absorb_blocks_avx512 (
    u64 *state,
    const u64 *rconst,
    const byte *lanes,
    u64 nlanes,
...
static unsigned int
keccak_absorb_lanes64_avx512 (KECCAK_STATE *hd,
                              int pos,
                              const byte *lanes,
                              size_t nlanes,
                              int blocklanes)
      ...
      if (pos == 0 && blocklanes > 0 && nlanes >= (size_t)blocklanes)
        {
          ...
          nlanes = _gcry_keccak_absorb_blocks_avx512 (
              hd->u.state64,
              _gcry_keccak_round_consts_64bit,
              lanes,
              nlanes,
              blocklanes,
              &new_lanes);
...
unsigned int _gcry_keccak_absorb_lanes64_armv7_neon (
    u64 *state, int pos, const byte *lanes, size_t nlanes, int blocklanes);

static unsigned int
keccak_permute64_armv7_neon (KECCAK_STATE *hd)
{
  return _gcry_keccak_permute_armv7_neon (hd->u.state64);
}

static unsigned int
keccak_absorb_lanes64_armv7_neon (KECCAK_STATE *hd,
                                  int pos,
                                  const byte *lanes,
                                  size_t nlanes,
                                  int blocklanes)
{
  if (blocklanes < 0)
    ...
    {
      return _gcry_keccak_absorb_lanes64_armv7_neon (
          hd->u.state64, pos, lanes, nlanes, blocklanes);
    }
}
...
#define ROL32(x, n)                                                           \
  (((x) << ((unsigned int)n & 31)) | ((x) >> ((32 - (unsigned int)(n)) & 31)))
...
static unsigned int
keccak_absorb_lanes32bi (KECCAK_STATE *hd,
                         int pos,
                         const byte *lanes,
                         size_t nlanes,
                         int blocklanes)
...
#define ANDN32(x, y)                                                          \
  ({                                                                          \
    u32 tmp;                                                                  \
    asm ("andnl %2, %1, %0" : "=r"(tmp) : "r0"(x), "rm"(y));                  \
    tmp;                                                                      \
  })
...
#define ROL32(x, n)                                                           \
  ({                                                                          \
    u32 tmp;                                                                  \
    asm ("rorxl %2, %1, %0" : "=r"(tmp) : "rm0"(x), "J"(32 - ((n)&31)));      \
    tmp;                                                                      \
  })
...
{
  x0 = pdep (pext (x0, 0x55555555), 0x0000ffff)
       | (pext (x0, 0xaaaaaaaa) << 16);
  x1 = pdep (pext (x1, 0x55555555), 0x0000ffff)
       | (pext (x1, 0xaaaaaaaa) << 16);
...
static unsigned int
keccak_absorb_lanes32bi_bmi2 (KECCAK_STATE *hd,
                              int pos,
                              const byte *lanes,
                              size_t nlanes,
                              int blocklanes)
...
static unsigned int
keccak_extract32bi_bmi2 (KECCAK_STATE *hd,
                         unsigned int pos,
                         byte *outbuf,
                         unsigned int outlen)
{
  ...
  for (i = pos; i < pos + outlen / 8 + !!(outlen % 8); i++)
    {
      ...
      x0 = pdep (pext (x0, 0xffff0001), 0xaaaaaaab)
           | pdep (x0 >> 1, 0x55555554);
      x1 = pdep (pext (x1, 0xffff0001), 0xaaaaaaab)
           | pdep (x1 >> 1, 0x55555554);
...
keccak_final_s390x (void *context)
{
  ...
  if (ctx->suffix == SHA3_DELIMITED_SUFFIX)
    ...
    {
      klmd_shake_execute (
          ctx->kimd_func, &ctx->state, NULL, 0, ctx->buf, ctx->count);
      ctx->count   = 0;
      ctx->buf_pos = 0;
    }
...
keccak_bextract_s390x (void *context, byte *out, size_t outlen)
{
  ...
  /* Extract full-blocks. */
  klmd_shake_execute (
      ctx->kimd_func | KLMD_PADDING_STATE, &ctx->state, out, outlen, NULL, 0);
  return;
}
  ...
  if (inlen && (count % 8))
    {
      ...
      nburn = ctx->ops->absorb (
          &ctx->state, pos, lane, 1, (count % 8) ? -1 : blocklanes);
...
static void
_gcry_sha3_hash_buffers (void *outbuf,
                         size_t nbytes,
...
static void
_gcry_sha3_224_hash_buffers (void *outbuf,
                             size_t nbytes,
                             const gcry_buffer_t *iov,
                             int iovcnt)
{
  _gcry_sha3_hash_buffers (
      outbuf, nbytes, iov, iovcnt, &_gcry_digest_spec_sha3_224);
}

static void
_gcry_sha3_256_hash_buffers (void *outbuf,
                             size_t nbytes,
                             const gcry_buffer_t *iov,
                             int iovcnt)
{
  _gcry_sha3_hash_buffers (
      outbuf, nbytes, iov, iovcnt, &_gcry_digest_spec_sha3_256);
}

static void
_gcry_sha3_384_hash_buffers (void *outbuf,
                             size_t nbytes,
                             const gcry_buffer_t *iov,
                             int iovcnt)
{
  _gcry_sha3_hash_buffers (
      outbuf, nbytes, iov, iovcnt, &_gcry_digest_spec_sha3_384);
}

static void
_gcry_sha3_512_hash_buffers (void *outbuf,
                             size_t nbytes,
                             const gcry_buffer_t *iov,
                             int iovcnt)
{
  _gcry_sha3_hash_buffers (
      outbuf, nbytes, iov, iovcnt, &_gcry_digest_spec_sha3_512);
}

static void
_gcry_shake128_hash_buffers (void *outbuf,
                             size_t nbytes,
                             const gcry_buffer_t *iov,
                             int iovcnt)
{
  _gcry_sha3_hash_buffers (
      outbuf, nbytes, iov, iovcnt, &_gcry_digest_spec_shake128);
}

static void
_gcry_shake256_hash_buffers (void *outbuf,
                             size_t nbytes,
                             const gcry_buffer_t *iov,
                             int iovcnt)
{
  _gcry_sha3_hash_buffers (
      outbuf, nbytes, iov, iovcnt, &_gcry_digest_spec_shake256);
}
...
selftests_keccak (int algo, int extended, selftest_report_func_t report)
{
  ...
  errtxt = _gcry_hash_selftest_check_one (
      algo, 0, "abc", 3, short_hash, hash_len);
  if (errtxt)
    goto failed;
  ...
  if (extended)
    {
      ...
      what   = "one million \"a\"";
      errtxt = _gcry_hash_selftest_check_one (
          algo, 1, NULL, 0, one_million_a_hash, hash_len);
      if (errtxt)
        goto failed;
    }
...
const gcry_md_spec_t _gcry_digest_spec_sha3_224 = {GCRY_MD_SHA3_224,
                                                   {0, 1},
                                                   "SHA3-224",
                                                   sha3_224_asn,
                                                   DIM (sha3_224_asn),
                                                   ...
                                                   run_selftests};
const gcry_md_spec_t _gcry_digest_spec_sha3_256 = {GCRY_MD_SHA3_256,
                                                   {0, 1},
                                                   "SHA3-256",
                                                   sha3_256_asn,
                                                   DIM (sha3_256_asn),
                                                   ...
                                                   sizeof (KECCAK_CONTEXT),
                                                   run_selftests};
const gcry_md_spec_t _gcry_digest_spec_sha3_384 = {GCRY_MD_SHA3_384,
                                                   {0, 1},
                                                   "SHA3-384",
                                                   sha3_384_asn,
                                                   DIM (sha3_384_asn),
                                                   ...
                                                   sizeof (KECCAK_CONTEXT),
                                                   run_selftests};
const gcry_md_spec_t _gcry_digest_spec_sha3_512 = {GCRY_MD_SHA3_512,
                                                   {0, 1},
                                                   "SHA3-512",
                                                   sha3_512_asn,
                                                   DIM (sha3_512_asn),
                                                   ...
                                                   sizeof (KECCAK_CONTEXT),
                                                   run_selftests};
const gcry_md_spec_t _gcry_digest_spec_shake128 = {GCRY_MD_SHAKE128,
                                                   {0, 1},
                                                   "SHAKE128",
                                                   shake128_asn,
                                                   DIM (shake128_asn),
                                                   ...
                                                   sizeof (KECCAK_CONTEXT),
                                                   run_selftests};
const gcry_md_spec_t _gcry_digest_spec_shake256 = {GCRY_MD_SHAKE256,
                                                   {0, 1},
                                                   "SHAKE256",
                                                   shake256_asn,
                                                   DIM (shake256_asn),
