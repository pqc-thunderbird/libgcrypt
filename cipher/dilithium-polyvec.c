...
void _gcry_dilithium_polyvec_matrix_expand(gcry_dilithium_param_t *params, gcry_dilithium_polyvec *mat, const uint8_t rho[GCRY_DILITHIUM_SEEDBYTES]) {
  ...
  for(i = 0; i < params->k; ++i)
    for(j = 0; j < params->l; ++j)
      _gcry_dilithium_poly_uniform(&mat[i].vec[j], rho, (i << 8) + j);
}
...
void _gcry_dilithium_polyvec_matrix_pointwise_montgomery(gcry_dilithium_param_t *params, gcry_dilithium_polyvec *t, const gcry_dilithium_polyvec *mat, const gcry_dilithium_polyvec *v) {
  ...
  for(i = 0; i < params->k; ++i)
    _gcry_dilithium_polyvecl_pointwise_acc_montgomery(params, &t->vec[i], &mat[i], v);
}
...
void _gcry_dilithium_polyvecl_uniform_eta(gcry_dilithium_param_t *params, gcry_dilithium_polyvec *v, const uint8_t seed[GCRY_DILITHIUM_CRHBYTES], uint16_t nonce) {
  ...
  for(i = 0; i < params->l; ++i)
    _gcry_dilithium_poly_uniform_eta(params, &v->vec[i], seed, nonce++);
}
...
void _gcry_dilithium_polyvecl_uniform_gamma1(gcry_dilithium_param_t *params, gcry_dilithium_polyvec *v, const uint8_t seed[GCRY_DILITHIUM_CRHBYTES], uint16_t nonce) {
  ...
  for(i = 0; i < params->l; ++i)
    _gcry_dilithium_poly_uniform_gamma1(params, &v->vec[i], seed, params->l*nonce + i);
}
...
void _gcry_dilithium_polyvecl_reduce(gcry_dilithium_param_t *params, gcry_dilithium_polyvec *v) {
  ...
  for(i = 0; i < params->l; ++i)
    _gcry_dilithium_poly_reduce(&v->vec[i]);
}

/*************************************************
* Name:        _gcry_dilithium_polyvecl_add
*
...
void _gcry_dilithium_polyvecl_add(gcry_dilithium_param_t *params, gcry_dilithium_polyvec *w, const gcry_dilithium_polyvec *u, const gcry_dilithium_polyvec *v) {
  ...
  for(i = 0; i < params->l; ++i)
    _gcry_dilithium_poly_add(&w->vec[i], &u->vec[i], &v->vec[i]);
}

/*************************************************
* Name:        _gcry_dilithium_polyvecl_ntt
*
...
void _gcry_dilithium_polyvecl_ntt(gcry_dilithium_param_t *params, gcry_dilithium_polyvec *v) {
  ...
  for(i = 0; i < params->l; ++i)
    _gcry_dilithium_poly_ntt(&v->vec[i]);
}
...
void _gcry_dilithium_polyvecl_pointwise_poly_montgomery(gcry_dilithium_param_t *params, gcry_dilithium_polyvec *r, const poly *a, const gcry_dilithium_polyvec *v) {
  ...
  for(i = 0; i < params->l; ++i)
    _gcry_dilithium_poly_pointwise_montgomery(&r->vec[i], a, &v->vec[i]);
}

/*************************************************
* Name:        _gcry_dilithium_polyvecl_pointwise_acc_montgomery
*
                                       ...
                                       const gcry_dilithium_polyvec *v)
{
  ...
  for(i = 1; i < params->l; ++i) {
    _gcry_dilithium_poly_pointwise_montgomery(&t, &u->vec[i], &v->vec[i]);
    _gcry_dilithium_poly_add(w, w, &t);
  }
}

/*************************************************
* Name:        _gcry_dilithium_polyvecl_chknorm
*
* Description: Check infinity norm of polynomials in vector of length params->l.
*              Assumes input gcry_dilithium_polyvec to be reduced by _gcry_dilithium_polyvecl_reduce().
*
...
* and 1 otherwise.
**************************************************/
int _gcry_dilithium_polyvecl_chknorm(gcry_dilithium_param_t *params, const gcry_dilithium_polyvec *v, int32_t bound)  {
  unsigned int i;

  for(i = 0; i < params->l; ++i)
    if(_gcry_dilithium_poly_chknorm(&v->vec[i], bound))
      return 1;
...
/*************************************************
* Name:        _gcry_dilithium_polyveck_reduce
*
...
/*************************************************
* Name:        _gcry_dilithium_polyveck_caddq
*
...
void _gcry_dilithium_polyveck_caddq(gcry_dilithium_param_t *params, gcry_dilithium_polyvec *v) {
  ...
  for(i = 0; i < params->k; ++i)
    _gcry_dilithium_poly_caddq(&v->vec[i]);
}

/*************************************************
* Name:        _gcry_dilithium_polyveck_add
*
...
/*************************************************
* Name:        _gcry_dilithium_polyveck_sub
*
...
void _gcry_dilithium_polyveck_sub(gcry_dilithium_param_t *params, gcry_dilithium_polyvec *w, const gcry_dilithium_polyvec *u, const gcry_dilithium_polyvec *v) {
  ...
  for(i = 0; i < params->k; ++i)
    _gcry_dilithium_poly_sub(&w->vec[i], &u->vec[i], &v->vec[i]);
}

/*************************************************
* Name:        _gcry_dilithium_polyveck_shiftl
*
...
void _gcry_dilithium_polyveck_shiftl(gcry_dilithium_param_t *params, gcry_dilithium_polyvec *v) {
  ...
  for(i = 0; i < params->k; ++i)
    _gcry_dilithium_poly_shiftl(&v->vec[i]);
}

/*************************************************
* Name:        _gcry_dilithium_polyveck_ntt
*
...
/*************************************************
* Name:        _gcry_dilithium_polyveck_invntt_tomont
*
...
/*************************************************
* Name:        _gcry_dilithium_polyveck_chknorm
*
* Description: Check infinity norm of polynomials in vector of length params->k.
*              Assumes input gcry_dilithium_polyvec to be reduced by _gcry_dilithium_polyveck_reduce().
*
* Arguments:   - const gcry_dilithium_polyvec *v: pointer to vector
*              - int32_t B: norm bound
*
...
* and 1 otherwise.
**************************************************/
int _gcry_dilithium_polyveck_chknorm(gcry_dilithium_param_t *params, const gcry_dilithium_polyvec *v, int32_t bound) {
  unsigned int i;
...
/*************************************************
* Name:        _gcry_dilithium_polyveck_power2round
*
...
void _gcry_dilithium_polyveck_power2round(gcry_dilithium_param_t *params, gcry_dilithium_polyvec *v1, gcry_dilithium_polyvec *v0, const gcry_dilithium_polyvec *v) {
  ...
  for(i = 0; i < params->k; ++i)
    _gcry_dilithium_poly_power2round(&v1->vec[i], &v0->vec[i], &v->vec[i]);
}

/*************************************************
* Name:        _gcry_dilithium_polyveck_decompose
*
* Description: For all coefficients a of polynomials in vector of length params->k,
...
void _gcry_dilithium_polyveck_decompose(gcry_dilithium_param_t *params, gcry_dilithium_polyvec *v1, gcry_dilithium_polyvec *v0, const gcry_dilithium_polyvec *v) {
  ...
  for(i = 0; i < params->k; ++i)
    _gcry_dilithium_poly_decompose(params, &v1->vec[i], &v0->vec[i], &v->vec[i]);
}

/*************************************************
* Name:        _gcry_dilithium_polyveck_make_hint
*
...
**************************************************/
unsigned int _gcry_dilithium_polyveck_make_hint(gcry_dilithium_param_t *params,
                                ...
                                const gcry_dilithium_polyvec *v1)
{
  ...
  for(i = 0; i < params->k; ++i)
    s += _gcry_dilithium_poly_make_hint(params, &h->vec[i], &v0->vec[i], &v1->vec[i]);

  return s;
}

/*************************************************
* Name:        _gcry_dilithium_polyveck_use_hint
*
...
**************************************************/
void _gcry_dilithium_polyveck_use_hint(gcry_dilithium_param_t *params, gcry_dilithium_polyvec *w, const gcry_dilithium_polyvec *u, const gcry_dilithium_polyvec *h) {
  unsigned int i;

  for(i = 0; i < params->k; ++i)
    _gcry_dilithium_poly_use_hint(params, &w->vec[i], &u->vec[i], &h->vec[i]);
}
...
void _gcry_dilithium_polyveck_pack_w1(gcry_dilithium_param_t *params, uint8_t *r, const gcry_dilithium_polyvec *w1) {
  ...
  for(i = 0; i < params->k; ++i)
    _gcry_dilithium_polyw1_pack(params, &r[i*params->polyw1_packedbytes], &w1->vec[i]);
}
