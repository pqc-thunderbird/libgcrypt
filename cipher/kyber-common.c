...
gcry_err_code_t crypto_kem_dec(uint8_t *ss, const uint8_t *ct, const uint8_t *sk, gcry_kyber_param_t *param)
{
  ...
  if ((ec = indcpa_enc(cmp, buf, pk, kr + GCRY_KYBER_SYMBYTES, param)))
    {
      goto end;
    }
  ...
  if ((ec = kyber_shake256_rkprf(ss, sk + param->secret_key_bytes - GCRY_KYBER_SYMBYTES, ct, param->ciphertext_bytes)))
    {
      goto end;
    }
    ...
    uint8_t *ct, uint8_t *ss, const uint8_t *pk, gcry_kyber_param_t *param, uint8_t *coins)
{
  ...
  /* Multitarget countermeasure for coins + contributory KEM */
  //_gcry_md_hash_buffer(GCRY_MD_SHA3_256, buf+GCRY_KYBER_SYMBYTES, pk, KYBER_PUBLICKEYBYTES);
  _gcry_md_hash_buffer(GCRY_MD_SHA3_256, buf + GCRY_KYBER_SYMBYTES, pk, param->public_key_bytes);
  //_gcry_md_hash_buffer(GCRY_MD_SHA3_512, kr, buf, 2*GCRY_KYBER_SYMBYTES);
  _gcry_md_hash_buffer(GCRY_MD_SHA3_512, kr, buf, 2 * GCRY_KYBER_SYMBYTES);

  /* coins are in kr+GCRY_KYBER_SYMBYTES */
  if ((ec = indcpa_enc(ct, buf, pk, kr + GCRY_KYBER_SYMBYTES, param)))
    {
      goto end;
    }
