 ...
 */
static gcry_err_code_t thash_shake_simple(
...
static gcry_err_code_t thash_512_simple(
    unsigned char *out, const unsigned char *in, unsigned int inblocks, const _gcry_slhdsa_param_t *ctx, u32 addr[8]);

/**
 * Takes an array of inblocks concatenated arrays of ctx->n bytes.
 */
static gcry_err_code_t thash_sha2_simple(
    unsigned char *out, const unsigned char *in, unsigned int inblocks, const _gcry_slhdsa_param_t *ctx, u32 addr[8])
{
  gcry_err_code_t ec = 0;
  gcry_md_hd_t hd    = NULL;
  ...
  /* copy hash context with already initialized pub seed */
